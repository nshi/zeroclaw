...
use mentat::memory::sqlite::SqliteMemory;
use mentat::memory::traits::{Memory, MemoryCategory};
use mentat::providers::ToolCall;
...
async fn compressor_with_memory_saves_summary() {
    use mentat::agent::context_compressor::{ContextCompressionConfig, ContextCompressor};
    use mentat::providers::traits::ChatMessage;
...
async fn consolidation_extracts_facts_to_memory() {
    ...
    let result = mentat::memory::consolidation::consolidate_turn(
        &provider,
        "test-model",
        mem.as_ref(),
...
async fn memory_survives_rapid_consolidation() {
    ...
    for i in 0..10 {
        ...
        let _ = mentat::memory::consolidation::consolidate_turn(
            &provider,
            "test-model",
            mem.as_ref(),
...
async fn session_backend_persists_messages() {
    use mentat::channels::session_backend::SessionBackend;
    use mentat::channels::session_sqlite::SqliteSessionBackend;
    use mentat::providers::traits::ChatMessage;
