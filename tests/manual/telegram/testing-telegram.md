...
This guide covers testing the Telegram channel integration for Mentat.
...
6. **Error logging**
    ...
    ```bash
    RUST_LOG=debug mentat channel start
    ```
...
Solution: Check user allowlist
  ...
  4. Run: mentat onboard --channels-only
```
...
| ---------------------- | ---------- | -------------------------------- |
| Health check time      | <5s        | `time mentat channel doctor`   |
| First response time    | <3s        | Time from sending to receiving   |
| Message split overhead | <50ms      | Check logs for timing            |
| Memory usage           | <10MB      | `ps aux \| grep mentat`        |
| Binary size            | ~3-4MB     | `ls -lh target/release/mentat` |
...
```bash
# Maximum logging
RUST_LOG=trace mentat channel start
...
# Validate config
cat ~/.mentat/config.toml | grep -A 3 "\[channels_config.telegram\]"
```
...
- [Mentat Main README](../../README.md)
- [Contributing Guide](../../CONTRIBUTING.md)
- [Issue Tracker](https://github.com/nshi/mentat/issues)
