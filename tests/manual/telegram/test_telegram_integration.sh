#!/bin/bash
# Mentat Telegram Integration Test Suite
...
if [ -f "target/release/mentat" ]; then
    BINARY_SIZE=$(ls -lh target/release/mentat | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/mentat 2>/dev/null || stat -c%s target/release/mentat)
...
CONFIG_PATH="$HOME/.mentat/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'mentat onboard' first"
    fi
else
    warn "No config file found - run 'mentat onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/mentat channel doctor 2>&1 || true)
END_TIME=$(date +%s)
