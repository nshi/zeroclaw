...
//!
//! Requires:
//! - Live Gemini OAuth profile in `~/.mentat/auth-profiles.json` with refresh_token
...
async fn gemini_warmup_refreshes_expired_oauth_token() -> Result<()> {
    // Find ~/.mentat/auth-profiles.json
    let home = env::var("HOME").expect("HOME env var not set");
    let mentat_dir = PathBuf::from(home).join(".mentat");
    let auth_profiles_path = mentat_dir.join("auth-profiles.json");
    ...
    if !auth_profiles_path.exists() {
        ...
        eprintln!("Run: mentat auth login --provider gemini");
        return Ok(());
    }
    ...
    let gemini_profile_key = profiles
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
                "No Gemini OAuth profile found. Run: mentat auth login --provider gemini"
            )
        })?
        .clone();
    ...
    // This will load auth from ~/.mentat/auth-profiles.json (with expired token)
    let provider = mentat::providers::create_provider("gemini", None)?;
