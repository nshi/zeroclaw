#!/usr/bin/env bash
# deploy-rpi.sh — cross-compile Mentat for Raspberry Pi and deploy via SSH.
#
...
#   RPI_DIR         — remote deployment dir           (default: /home/$RPI_USER/mentat)
...
RPI_DIR="${RPI_DIR:-/home/${RPI_USER}/mentat}"
TARGET="aarch64-unknown-linux-gnu"
FEATURES="hardware,peripheral-rpi"
BINARY="target/${TARGET}/release/mentat"
...
echo "==> Building Mentat for Raspberry Pi (${TARGET})"
...
echo ""
echo "==> Stopping mentat service (if running)"
# shellcheck disable=SC2029
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" \
  "sudo systemctl stop mentat 2>/dev/null || true"
...
# ── 4. Deploy binary ──────────────────────────────────────────────────────────
echo ""
echo "==> Deploying binary to ${RPI_USER}@${RPI_HOST}:${RPI_DIR}/mentat"
${SCP_CMD} ${SCP_OPTS} "${BINARY}" "${RPI_USER}@${RPI_HOST}:${RPI_DIR}/mentat"
...
# ── 5. Deploy config ─────────────────────────────────────────────────────────
CONFIG_DEST="/home/${RPI_USER}/.mentat/config.toml"
echo ""
echo "==> Deploying config to ${CONFIG_DEST}"
# shellcheck disable=SC2029
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" "mkdir -p /home/${RPI_USER}/.mentat"
...
SERVICE_DEST="/etc/systemd/system/mentat.service"
echo ""
echo "==> Installing systemd service (requires sudo on the Pi)"
${SCP_CMD} ${SCP_OPTS} "scripts/mentat.service" "${RPI_USER}@${RPI_HOST}:/tmp/mentat.service"
# shellcheck disable=SC2029
${SSH_CMD} ${SSH_OPTS} "${RPI_USER}@${RPI_HOST}" \
  "sudo mv /tmp/mentat.service ${SERVICE_DEST} && \
   sudo systemctl daemon-reload && \
   sudo systemctl enable mentat && \
   sudo systemctl restart mentat && \
   sudo systemctl status mentat --no-pager || true"
...
# ── 8. Reset ACT LED trigger so Mentat can control it ──────────────────────
echo ""
...
echo ""
echo "    Mentat is running at http://${RPI_HOST}:8080"
echo "    POST /api/chat  — chat with the agent"
echo "    GET  /health    — health check"
echo ""
echo "    To check logs: ssh ${RPI_USER}@${RPI_HOST} 'journalctl -u mentat -f'"
