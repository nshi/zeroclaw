...
This directory contains everything needed to cross-compile Mentat and deploy it to a Raspberry Pi over SSH.
...
| `rpi-config.toml` | Production config template deployed to `~/.mentat/config.toml` |
| `mentat.service` | systemd unit file installed on the Pi |
...
| `RPI_DIR` | `~/mentat` | Remote directory for the binary and `.env` |
...
2. **Stop service** — runs `sudo systemctl stop mentat` on the Pi (continues if not yet installed).
3. **Create remote directory** — ensures `$RPI_DIR` exists on the Pi.
4. **Copy binary** — SCPs the compiled binary to `$RPI_DIR/mentat`.
...
6. **Deploy config** — copies `rpi-config.toml` to `~/.mentat/config.toml`, preserving any `api_key` already present in the file.
7. **Install systemd service** — copies `mentat.service` to `/etc/systemd/system/`, then enables and restarts it.
...
## Interacting with Mentat on the Pi
...
```bash
ssh pi@raspberrypi.local 'journalctl -u mentat -f'
```
...
Mentat is deployed with the `peripheral-rpi` feature, which enables two LLM-callable tools:
...
|------------|--------|-------------|
| `~/mentat/mentat` | compiled binary | Main agent binary |
| `~/mentat/.env` | created on first deploy | API key and environment variables |
| `~/.mentat/config.toml` | `rpi-config.toml` | Agent configuration |
| `/etc/systemd/system/mentat.service` | `mentat.service` | systemd service unit |
...
To customise, edit `~/.mentat/config.toml` directly on the Pi and restart the service.
...
```bash
ssh pi@raspberrypi.local 'sudo systemctl status mentat'
ssh pi@raspberrypi.local 'journalctl -u mentat -n 50 --no-pager'
```
...
```bash
ssh pi@raspberrypi.local 'file ~/mentat/mentat'
