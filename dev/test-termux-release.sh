...
ASSET_URL="https://github.com/nshi/mentat/releases/download/${TAG}/${ASSET_NAME}"
TEMP_DIR="$(mktemp -d -t mentat-termux-test-XXXXXX)"
...
# --- Test 5: Contains mentat binary ---
info "Checking archive contents"
CONTENTS=$(tar -tzf "$TEMP_DIR/$ASSET_NAME")
if echo "$CONTENTS" | grep -q "^mentat$"; then
  pass "Archive contains 'mentat' binary"
else
  fail "Archive does not contain 'mentat' binary"
  echo "Contents:"
  echo "$CONTENTS" | sed 's/^/  /'
fi
...
CHECKSUMS_URL="https://github.com/nshi/mentat/releases/download/${TAG}/SHA256SUMS"
