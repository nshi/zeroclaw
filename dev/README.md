# Mentat Development Environment

A fully containerized development sandbox for Mentat agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`mentat-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `mentat` CLI commands manually, debug the binary, or check logs internally.
...
The agent configuration lives in `target/.mentat` (mounted to `/mentat-data/.mentat`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.mentat` (config/DB) but leaves the `playground/` directory intact. To fully wipe workspace data, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `mentat-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=mentat-target`) to speed repeat local image builds.
