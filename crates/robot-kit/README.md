...
A complete toolkit for building AI-powered robots with Mentat. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build Mentat with robot tools

```bash
# Clone and build
git clone https://github.com/nshi/mentat
cd mentat
cargo build -p mentat-robot-kit --release
```
...
# Copy config
mkdir -p ~/.mentat
cp crates/robot-kit/robot.toml ~/.mentat/
cp crates/robot-kit/SOUL.md ~/.mentat/workspace/
...
# Test in mock mode
./target/release/mentat agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/mentat agent -m "Move forward 1 meter"
```
...
```rust
use mentat_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `mentat`, add a thin adapter that maps this
...
mkdir mentat-robot-kit
cp -r target/release/mentat mentat-robot-kit/
cp -r examples/robot_kit mentat-robot-kit/
cp -r ~/.mentat mentat-robot-kit/dot-mentat

# Include models
mkdir -p mentat-robot-kit/models
cp ~/.mentat/models/ggml-base.bin mentat-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf mentat-robot-kit.tar.gz mentat-robot-kit/

# Copy to USB
cp mentat-robot-kit.tar.gz /media/usb/TarBalls/
```
