...
Complete guide to setting up a Mentat-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.mentat/models
cp models/ggml-base.bin ~/.mentat/models/
```
...
mkdir -p ~/.mentat/models/piper
cd ~/.mentat/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.mentat/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
### 7. Build Mentat Robot Kit

```bash
# Clone repo (or copy from USB)
git clone https://github.com/nshi/mentat
cd mentat

# Build robot kit
cargo build --release -p mentat-robot-kit

# Build main mentat (optional, if using as agent)
cargo build --release
```
...
```bash
mkdir -p ~/.mentat
nano ~/.mentat/robot.toml
```

```toml
# ~/.mentat/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.mentat/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/mentat_sensors.fifo"
...
```bash
# Configure Mentat to use robot tools
cat > ~/.mentat/config.toml << 'EOF'
...
cp ~/mentat/crates/robot-kit/SOUL.md ~/.mentat/workspace/

# Start agent
./target/release/mentat agent
```
...
# Start sensor loop
if [ ! -p /tmp/mentat_sensors.fifo ]; then
    mkfifo /tmp/mentat_sensors.fifo
fi
...
```bash
# /etc/systemd/system/mentat-robot.service
sudo tee /etc/systemd/system/mentat-robot.service << 'EOF'
[Unit]
Description=Mentat Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/mentat
...
sudo systemctl enable mentat-robot
sudo systemctl start mentat-robot

# Check status
sudo systemctl status mentat-robot
journalctl -u mentat-robot -f  # View logs
```
