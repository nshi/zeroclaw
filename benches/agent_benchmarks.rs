//! Performance benchmarks for Mentat hot paths.
//!
//! Benchmarks cover:
...
//! Run: `cargo bench`
//!
//! Ref: https://github.com/nshi/mentat/issues/618 (item 7)
...
use mentat::agent::dispatcher::{NativeToolDispatcher, ToolDispatcher, XmlToolDispatcher};
use mentat::config::MemoryConfig;
use mentat::memory;
use mentat::memory::{Memory, MemoryCategory};
use mentat::observability::{NoopObserver, Observer};
use mentat::providers::{ChatRequest, ChatResponse, Provider, ToolCall};
use mentat::tools::{Tool, ToolResult};
...
<tool_call>
{"name": "search", "arguments": {"query": "mentat architecture"}}
</tool_call>
...
fn bench_memory_operations(c: &mut Criterion) {
    ...
    rt.block_on(async {
        for i in 0..100 {
            mem.store(
                &format!("key_{i}"),
                &format!("Content entry number {i} about mentat agent runtime"),
                MemoryCategory::Core,
                None,
            )
            .await
    ...
    c.bench_function("memory_recall_top10", |b| {
        b.iter(|| {
            rt.block_on(async {
                mem.recall(black_box("mentat agent"), 10, None, None, None)
                    .await
                    .unwrap()
            })
        });
