...
fn show_integration_info(config: &Config, name: &str) -> Result<()> {
    ...
    let Some(entry) = entries.iter().find(|e| e.name.to_lowercase() == name_lower) else {
        anyhow::bail!(
            "Unknown integration: {name}. Check README for supported integrations or run `mentat onboard` to configure channels/providers."
        );
    };
    ...
    match entry.name {
        "Telegram" => {
            ...
            println!("    3. Run: mentat onboard --channels-only");
            println!("    4. Start: mentat channel start");
        }
        "Slack" => {
            println!("  Setup:");
        ...
        "OpenRouter" => {
            ...
            println!("    2. Run: mentat onboard");
            println!("    Access 200+ models with one key.");
        }
        "Browser" => {
            println!("  Built-in:");
            println!("    Mentat can control Chrome/Chromium for web tasks.");
            println!("    Uses headless browser automation.");
        }
        "Cron" => {
            println!("  Built-in:");
            println!("    Schedule tasks in ~/.mentat/workspace/cron/");
            println!("    Run: mentat cron list");
        }
        "Weather" => {
            println!("  Built-in:");
        ...
        _ => {
            if status == IntegrationStatus::ComingSoon {
                ...
                println!("  Track progress: https://github.com/nshi/mentat");
            }
        }
    }
