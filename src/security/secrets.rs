...
// in `~/.mentat/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.mentat/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(mentat_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: mentat_dir.join(".secret_key"),
            enabled,
        }
    }
