//! Wraps a discovered MCP tool as a mentat [`Tool`] so it is dispatched
...
/// A mentat [`Tool`] backed by an MCP server tool.
///
...
impl Tool for McpToolWrapper {
    ...
    async fn execute(&self, args: serde_json::Value) -> anyhow::Result<ToolResult> {
        ...
        // Mentat's security model injects `approved: bool` into built-in tool
...
mod tests {
    ...
    // Mentat's security model injects `approved: bool` into built-in tool args.
