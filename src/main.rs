...
fn print_no_command_help() -> Result<()> {
    ...
    println!("Try `mentat onboard` to initialize your workspace.");
    println!();
...
/// `Mentat` - Zero overhead. Zero compromise. 100% Rust.
#[derive(Parser, Debug)]
#[command(name = "mentat")]
...
Examples:
  mentat agent                              # interactive session
  mentat agent -m \"Summarize today's logs\"  # single message
  mentat agent -p anthropic --model claude-sonnet-4-20250514")]
    Agent {
...
Examples:
  mentat gateway start              # start gateway
  mentat gateway restart            # restart gateway
  mentat gateway get-paircode       # show pairing code")]
    Gateway {
        #[command(subcommand)]
        gateway_command: Option<mentat::GatewayCommands>,
    },
...
Examples:
  mentat acp                        # start ACP server
  mentat acp --max-sessions 5       # limit concurrent sessions")]
    Acp {
...
Launches the full Mentat runtime: gateway server, all configured \
channels (Telegram, Slack, CLI), heartbeat monitor, and \
the cron scheduler. This is the recommended way to run Mentat in \
production or as an always-on assistant.

Use 'mentat service install' to register the daemon as an OS \
service (systemd/launchd) for auto-start on boot.

Examples:
  mentat daemon                   # use config defaults
  mentat daemon -p 9090           # gateway on port 9090
  mentat daemon --host 127.0.0.1  # localhost only")]
    Daemon {
    ...
    /// - `mentat estop --level domain-block --domain "*.chase.com"`
    ...
    Estop {
        ...
        /// Level used when engaging estop from `mentat estop`.
        #[arg(long, value_enum)]
...
Examples:
  ...
  mentat cron add-at 2025-01-15T14:00:00Z 'Send reminder' --agent
  mentat cron add-every 60000 'Ping heartbeat'
  mentat cron once 30m 'Run backup in 30 minutes' --agent
  mentat cron pause <task-id>
  mentat cron update <task-id> --expression '0 8 * * *' --tz Europe/London")]
    Cron {
        #[command(subcommand)]
...
Add, remove, list, send, and health-check channels that connect Mentat \
...
Examples:
  ...
  mentat channel remove my-bot
  mentat channel bind-telegram mentat_user
  mentat channel send 'Alert!' --channel-id telegram --recipient 123456789")]
    Channel {
        #[command(subcommand)]
...
Examples:
  mentat memory stats
  mentat memory list
  mentat memory list --category core --limit 10
  mentat memory get <key>
  mentat memory clear --category conversation --yes")]
    Memory {
        #[command(subcommand)]
...
Examples:
  mentat config schema              # print JSON Schema to stdout
  mentat config schema > schema.json")]
    Config {
        #[command(subcommand)]
    ...
    #[command(long_about = "\
Check for and apply Mentat updates.
...
Examples:
  mentat update                      # download and install latest
  mentat update --check              # check only, don't install
  mentat update --force              # install without confirmation
  mentat update --version 0.6.0      # install specific version")]
    Update {
    ...
    #[command(long_about = "\
Run diagnostic self-tests to verify the Mentat installation.
...
Examples:
  mentat self-test             # full suite
  mentat self-test --quick     # quick checks only (no network)")]
    SelfTest {
    ...
    #[command(long_about = "\
Generate shell completion scripts for `mentat`.
...
Examples:
  ...
  mentat completions fish > ~/.config/fish/completions/mentat.fish")]
    Completions {
        /// Target shell
        #[arg(value_enum)]
    ...
    #[command(long_about = "\
Launch the Mentat companion desktop app.
...
Examples:
  mentat desktop              # launch the companion app
  mentat desktop --install    # download and install it")]
    Desktop {
...
async fn main() -> Result<()> {
    ...
    if let Some(config_dir) = &cli.config_dir {
        ...
        unsafe { std::env::set_var("MENTAT_CONFIG_DIR", config_dir) };
    }
    ...
    // or set MENTAT_INTERACTIVE=1 to force interactive mode when TTY
    // detection fails.  This means `curl … | bash` and
    // `mentat onboard --api-key …` both take the fast path, while a bare
    // `mentat onboard` in a terminal launches the wizard.
    ...
    } = &cli.command
    {
        ...
        if reinit {
            ...
            if mentat_dir.exists() {
                let timestamp = chrono::Local::now().format("%Y%m%d%H%M%S");
                let backup_dir = format!("{}.backup.{}", mentat_dir.display(), timestamp);

                println!("⚠️  Reinitializing Mentat configuration...");
                println!("   Current config directory: {}", mentat_dir.display());
                println!(
                ...
                tokio::fs::rename(&mentat_dir, &backup_dir)
                    .await
                    .with_context(|| {
        ...
        let env_interactive = std::env::var("MENTAT_INTERACTIVE").as_deref() == Ok("1");
        ...
        if std::env::var("MENTAT_AUTOSTART_CHANNELS").as_deref() == Ok("1") {
    ...
    if config.security.otp.enabled {
        ...
        if let Some(uri) = enrollment_uri {
            println!("Initialized OTP secret for Mentat.");
    ...
    match cli.command {
        ...
        Commands::Gateway { gateway_command } => {
            match gateway_command {
                Some(mentat::GatewayCommands::Restart { port, host }) => {
                    let (port, host) = resolve_gateway_addr(&config, port, host);
                    let addr = format!("{host}:{port}");
                    info!("🔄 Restarting Mentat Gateway on {addr}");
                ...
                Some(mentat::GatewayCommands::GetPaircode { new }) => {
                    ...
                    match fetch_paircode(host, port, new).await {
                        ...
                        Err(e) => {
                            ...
                            println!("     mentat gateway start");
                        }
                    }
                    Ok(())
                }
                Some(mentat::GatewayCommands::Start { port, host }) => {
                    let (port, host) = resolve_gateway_addr(&config, port, host);
                    log_gateway_start(&host, port);
                    Box::pin(gateway::run_gateway(&host, port, config)).await
                }
        ...
        Commands::Daemon { port, host } => {
            ...
            if port == 0 {
                info!("🧠 Starting Mentat Daemon on {host} (random port)");
            } else {
                info!("🧠 Starting Mentat Daemon on {host}:{port}");
            }
        ...
        Commands::Desktop {
            ...
            let download_url = "https://github.com/nshi/mentat/download";

            if do_install {
                println!("Download the Mentat companion app:");
                println!();
                #[cfg(target_os = "macos")]
                {
                    ...
                    println!("  brew install --cask mentat");
                }
                #[cfg(target_os = "linux")]
                {
            ...
            let desktop_bin = {
                ...
                // 1. macOS: check /Applications/Mentat.app
                #[cfg(target_os = "macos")]
                {
                    let app_paths = [
                        PathBuf::from("/Applications/Mentat.app/Contents/MacOS/Mentat"),
                        PathBuf::from(std::env::var("HOME").unwrap_or_default())
                            .join("Applications/Mentat.app/Contents/MacOS/Mentat"),
                    ];
                ...
                if found.is_none() {
                    if let Ok(exe) = std::env::current_exe() {
                        let sibling = exe.with_file_name("mentat-desktop");
                        if sibling.is_file() {
                            found = Some(sibling);
                        }
                    }
                ...
                // 3. ~/.cargo/bin/mentat-desktop or ~/.local/bin/mentat-desktop
                if found.is_none() {
                    if let Some(home) = std::env::var_os("HOME") {
                        let home = PathBuf::from(home);
                        for dir in &[".cargo/bin", ".local/bin"] {
                            let candidate = home.join(dir).join("mentat-desktop");
                ...
                if found.is_none() {
                    if let Ok(path) = which::which("mentat-desktop") {
                        found = Some(path);
                    }
                }
            ...
            match desktop_bin {
                Some(bin) => {
                    println!("Launching Mentat companion app...");
                ...
                None => {
                    println!("Mentat companion app is not installed.");
                    println!();
                    println!("  Download it at: {download_url}");
                    println!("  Or run: mentat desktop --install");
                    println!();
        ...
        Commands::Plugin { plugin_command } => match plugin_command {
            PluginCommands::List => {
                let host = mentat::plugins::host::PluginHost::new(&config.workspace_dir)?;
            ...
            PluginCommands::Install { source } => {
                let mut host = mentat::plugins::host::PluginHost::new(&config.workspace_dir)?;
                host.install(&source)?;
...
fn log_gateway_start(host: &str, port: u16) {
    if port == 0 {
        info!("🚀 Starting Mentat Gateway on {host} (random port)");
    } else {
        info!("🚀 Starting Mentat Gateway on {host}:{port}");
    }
}
                ...
                "gemini" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            ...
                            println!(
                                "Run `mentat auth paste-redirect --provider gemini --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
                ...
                "openai-codex" => {
                    ...
                    .await
                    {
                        ...
                        Err(e) => {
                            println!("Callback capture failed: {e}");
                            println!(
                                "Run `mentat auth paste-redirect --provider openai-codex --profile {profile}`"
                            );
                            return Ok(());
                        }
                    };
            ...
            match provider.as_str() {
                "openai-codex" => {
                    let pending = load_pending_oauth_login(config, "openai")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending OpenAI login found. Run `mentat auth login --provider openai-codex` first."
                        )
                    })?;
                ...
                "gemini" => {
                    let pending = load_pending_oauth_login(config, "gemini")?.ok_or_else(|| {
                        anyhow::anyhow!(
                            "No pending Gemini login found. Run `mentat auth login --provider gemini` first."
                        )
                    })?;
            ...
            match provider.as_str() {
                "openai-codex" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No OpenAI Codex auth profile found. Run `mentat auth login --provider openai-codex`."
                            )
                        }
                    }
                }
                "gemini" => {
                        ...
                        .await?
                    {
                        ...
                        None => {
                            bail!(
                                "No Gemini auth profile found. Run `mentat auth login --provider gemini`."
                            )
                        }
                    }
                }
...
mod tests {
    ...
    fn completions_cli_parses_supported_shells() {
        for shell in ["bash", "fish", "zsh", "powershell", "elvish"] {
            let cli = Cli::try_parse_from(["mentat", "completions", shell])
    ...
    fn onboard_cli_accepts_force_flag() {
        let cli = Cli::try_parse_from(["mentat", "onboard", "--force"])
    ...
    fn onboard_cli_rejects_removed_interactive_flag() {
        ...
        assert!(Cli::try_parse_from(["mentat", "onboard", "--interactive"]).is_err());
    }

    #[test]
    fn onboard_cli_parses_quick_flag() {
        let cli = Cli::try_parse_from(["mentat", "onboard", "--quick"])
    ...
    fn onboard_cli_quick_and_channels_only_conflict() {
        ...
        let cli = Cli::try_parse_from(["mentat", "onboard", "--quick", "--channels-only"]);
        assert!(
            cli.is_ok(),
    ...
    fn onboard_cli_bare_parses() {
        let cli = Cli::try_parse_from(["mentat", "onboard"]).expect("bare onboard should parse");
    ...
    fn cli_parses_estop_default_engage() {
        let cli = Cli::try_parse_from(["mentat", "estop"]).expect("estop command should parse");
    ...
    fn cli_parses_estop_resume_domain() {
        let cli = Cli::try_parse_from(["mentat", "estop", "resume", "--domain", "*.chase.com"])
    ...
    fn agent_command_parses_with_temperature() {
        let cli = Cli::try_parse_from(["mentat", "agent", "--temperature", "0.5"])
    ...
    fn agent_command_parses_without_temperature() {
        let cli = Cli::try_parse_from(["mentat", "agent", "--message", "hello"])
    ...
    fn agent_command_parses_session_state_file() {
        let cli = Cli::try_parse_from(["mentat", "agent", "--session-state-file", "session.json"])
