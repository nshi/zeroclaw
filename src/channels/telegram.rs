use super::traits::{Channel, ChannelMessage, InboxInterceptor, NextReply, SendMessage};
...
pub struct TelegramChannel {
    ...
    /// Diverts targeted inbound messages from the supervised listener to
    /// awaiting tools (e.g. `ask_user`) so they aren't processed as new
    /// turns. Shared logic lives in `traits::InboxInterceptor`.
    interceptor: InboxInterceptor,
}
...
impl TelegramChannel {
    pub fn new(bot_token: String, allowed_users: Vec<String>, mention_only: bool) -> Self {
        ...
        Self {
            ...
            proxy_url: None,
            interceptor: InboxInterceptor::new(),
        }
    }
    ...
    async fn load_config_without_env() -> anyhow::Result<Config> {
        ...
        config.workspace_dir = mentat_dir.join("workspace");
        Ok(config)
    }
    ...
    async fn persist_allowed_identity(&self, identity: &str) -> anyhow::Result<()> {
        ...
        let Some(telegram) = config.channels_config.telegram.as_mut() else {
            anyhow::bail!(
                ...
                or run `mentat onboard --channels-only` to configure interactively"
            );
        };
                                ...
                                Ok(()) => {
                                    let _ = self
                                        .send(&SendMessage::new(
                                            "✅ Telegram account bound successfully. You can talk to Mentat now.",
                                            &chat_id,
                                        ))
                                        .await;
                                    tracing::info!(
...
Ensure only one `mentat` process is using this bot token."
                    );
            ...
            if let Some(results) = data.get("result").and_then(serde_json::Value::as_array) {
                for update in results {
                    ...
                    if let Some(sender) = self.interceptor.take(&msg.reply_target) {
    ...
    async fn await_next_reply(
        ...
        self.interceptor.await_reply(reply_target, timeout).await
    }
...
mod tests {
    ...
    async fn await_next_reply_times_out_without_match() {
        ...
        assert!(ch.interceptor.is_empty());
    }
    ...
    fn telegram_extract_bind_code_supports_bot_mention() {
        assert_eq!(
            TelegramChannel::extract_bind_code("/bind@mentat_bot 654321"),
            Some("654321")
        );
    }
