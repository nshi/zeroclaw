...
//! Mentat to external platforms. Each channel implements the [`Channel`] trait
...
const SYSTEMD_STATUS_ARGS: [&str; 3] = ["--user", "is-active", "mentat.service"];
const SYSTEMD_RESTART_ARGS: [&str; 3] = ["--user", "restart", "mentat.service"];
const OPENRC_STATUS_ARGS: [&str; 2] = ["mentat", "status"];
const OPENRC_RESTART_ARGS: [&str; 2] = ["mentat", "restart"];
...
async fn load_runtime_defaults_from_config_file(path: &Path) -> Result<ChannelRuntimeDefaults> {
    ...
    if let Some(mentat_dir) = path.parent() {
        let store = crate::security::SecretStore::new(mentat_dir, parsed.secrets.encrypt);
...
fn build_models_help_response(
    ...
    if cached_models.is_empty() {
        let _ = writeln!(
            response,
            "\nNo cached model list found for `{}`. Ask the operator to run `mentat models refresh --provider {}`.",
...
const BLOCK_KIT_PREFIX: &str = "__MENTAT_BLOCK_KIT__";
...
fn build_config_block_kit(
    ...
    let mut provider_select = serde_json::json!({
        "type": "static_select",
        "action_id": "mentat_config_provider",
    ...
    let response = match command {
        ...
        ChannelRuntimeCommand::ShowConfig => {
            if msg.channel == "slack" {
                ...
                format!("__MENTAT_BLOCK_KIT__{blocks_json}")
            } else {
...
async fn bind_telegram_identity(config: &Config, identity: &str) -> Result<()> {
    ...
    let Some(telegram) = updated.channels_config.telegram.as_mut() else {
        anyhow::bail!(
            "Telegram channel is not configured. Run `mentat onboard --channels-only` first"
        );
    };
    ...
    match maybe_restart_managed_daemon_service() {
        ...
        Ok(false) => {
            println!(
                "ℹ️ No managed daemon service detected. If `mentat daemon`/`channel start` is already running, restart it to load the updated allowlist."
            );
        }
        Err(e) => {
            eprintln!(
                "⚠️ Allowlist saved, but failed to reload daemon service automatically: {e}\n\
                 Restart service manually with `mentat service stop && mentat service start`."
            );
        }
    }
    Ok(())
...
fn maybe_restart_managed_daemon_service() -> Result<bool> {
    if cfg!(target_os = "macos") {
        ...
        if !listed.contains("com.mentat.daemon") {
            return Ok(false);
        }

        let _ = Command::new("launchctl")
            .args(["stop", "com.mentat.daemon"])
            .output();
        let start_output = Command::new("launchctl")
            .args(["start", "com.mentat.daemon"])
            .output()
    ...
    if cfg!(target_os = "linux") {
        ...
        let openrc_init_script = PathBuf::from("/etc/init.d/mentat");
...
pub(crate) async fn handle_command(command: crate::ChannelCommands, config: &Config) -> Result<()> {
    match command {
        ...
        crate::ChannelCommands::List => {
            ...
            println!("\nTo start channels: mentat channel start");
            println!("To check health:    mentat channel doctor");
            println!("To configure:      mentat onboard");
            Ok(())
        }
        crate::ChannelCommands::Add {
            ...
            anyhow::bail!(
                "Channel type '{channel_type}' — use `mentat onboard` to configure channels"
            );
        }
        crate::ChannelCommands::Remove { name } => {
            anyhow::bail!("Remove channel '{name}' — edit ~/.mentat/config.toml directly");
        }
...
fn build_channel_by_id(config: &Config, channel_id: &str) -> Result<Arc<dyn Channel>> {
    match channel_id {
        ...
        other => anyhow::bail!("Unknown channel '{other}'. Supported: telegram, slack"),
    }
}
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() {
        println!("No real-time channels configured. Run `mentat onboard` first.");
        return Ok(());
    }

    println!("🩺 Mentat Channel Doctor");
    println!();
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    let provider_runtime_options = providers::ProviderRuntimeOptions {
        ...
        mentat_dir: config.config_path.parent().map(std::path::PathBuf::from),
    ...
    let channels: Vec<Arc<dyn Channel>> = collect_configured_channels(&config)
        .into_iter()
        .map(|configured| configured.channel)
        .collect();

    if channels.is_empty() {
        println!("No channels configured. Run `mentat onboard` to set up channels.");
        return Ok(());
    }

    println!("🦀 Mentat Channel Server");
    println!("  🤖 Model:    {model}");
...
BTC is currently around $65,000 based on latest tool output."#
    ...
    async fn process_channel_message_uses_runtime_default_model_from_store() {
        ...
        let runtime_ctx = Arc::new(ChannelRuntimeContext {
            ...
            reliability: Arc::new(crate::config::ReliabilityConfig::default()),
            provider_runtime_options: providers::ProviderRuntimeOptions {
                mentat_dir: Some(temp.path().to_path_buf()),
    ...
    fn prompt_injects_workspace_files() {
        ...
        let prompt = make_prompt(ws.path(), "model");

        assert!(prompt.contains("### SOUL.md"), "missing SOUL.md header");
        assert!(prompt.contains("Be helpful"), "missing SOUL content");
        assert!(prompt.contains("### IDENTITY.md"), "missing IDENTITY.md");
        assert!(
            prompt.contains("Name: Mentat"),
            "missing IDENTITY content"
        );
        assert!(prompt.contains("### USER.md"), "missing USER.md");
        assert!(prompt.contains("### AGENTS.md"), "missing AGENTS.md");
        assert!(prompt.contains("### TOOLS.md"), "missing TOOLS.md");
        // The new personality module includes HEARTBEAT.md (unlike the old builder).
        assert!(
            prompt.contains("### HEARTBEAT.md"),
            "HEARTBEAT.md should be included by the personality module"
        );
    ...
    fn prompt_bootstrap_only_if_exists() {
        ...
        let prompt2 = make_prompt(ws.path(), "model");
        assert!(
    ...
    fn prompt_runtime_metadata() {
        let ws = make_workspace();
        let prompt = make_prompt(ws.path(), "claude-sonnet-4");
    ...
    fn prompt_truncation() {
        let ws = make_workspace();
        // Write a file larger than the personality module's MAX_FILE_CHARS (20_000)
        let big_content = "x".repeat(20_000 + 1000);
    ...
    fn channel_log_truncation_is_utf8_safe_for_multibyte_text() {
        let msg = "Hello from Mentat 🌍. Current status is healthy, and café-style UTF-8 text stays safe in logs.";
...
This is an example JSON object for profile settings."#;
    ...
    fn maybe_restart_daemon_systemd_args_regression() {
        assert_eq!(
            SYSTEMD_STATUS_ARGS,
            ["--user", "is-active", "mentat.service"]
        );
        assert_eq!(
            SYSTEMD_RESTART_ARGS,
            ["--user", "restart", "mentat.service"]
        );
    }

    #[test]
    fn maybe_restart_daemon_openrc_args_regression() {
        assert_eq!(OPENRC_STATUS_ARGS, ["mentat", "status"]);
        assert_eq!(OPENRC_RESTART_ARGS, ["mentat", "restart"]);
    }
    ...
    async fn e2e_failed_vision_turn_does_not_poison_follow_up_text_turn() {
        ...
        let histories = runtime_ctx
            ...
            .unwrap_or_else(|e| e.into_inner());
        let turns = histories
            .get("test-channel_chat-photo_mentat_user")
    ...
    async fn e2e_failed_non_retryable_turn_does_not_poison_follow_up_text_turn() {
        ...
        let histories = runtime_ctx
            ...
            .unwrap_or_else(|e| e.into_inner());
        let turns = histories
            .get("test-channel_chat-format_mentat_user")
            .expect("history should exist for sender");
        assert_eq!(turns.len(), 2);
        assert_eq!(turns[0].role, "user");
        assert!(
    ...
    fn is_stop_command_matches_with_bot_suffix() {
        assert!(is_stop_command("/stop@mentat_bot"));
    }
