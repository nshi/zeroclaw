...
/// - `X-Mentat-Timestamp` — unix epoch seconds
/// - `X-Mentat-Nonce` — random UUID v4
/// - `X-Mentat-Signature` — HMAC-SHA256 hex digest
///
...
impl NodeTransport {
    ...
    pub async fn send(
        ...
        let resp = self
            .http
            .post(&url)
            .header("X-Mentat-Timestamp", timestamp.to_string())
            .header("X-Mentat-Nonce", &nonce)
            .header("X-Mentat-Signature", &signature)
