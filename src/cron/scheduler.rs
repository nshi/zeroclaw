...
async fn run_agent_job(
    ...
    // The agent loop injects memory context downstream (see `build_context`
    // in src/agent/loop_.rs); doing it here too would double-wrap the user
    // message and bias recall toward the cron prompt's own history. The
    // CRON_MSG_PREFIX marker is what `is_synthetic_trigger_content` matches
    // on to skip both autosave and recall — keep it on the outside.
    let prefixed_prompt = format!(
        "{}{} {name}] {prompt}",
        crate::memory::CRON_MSG_PREFIX,
        job.id,
    );
...
async fn run_job_command_with_timeout(
    ...
    if let Err(error) = crate::cron::validate_shell_command_with_security(security, &job.command) {
