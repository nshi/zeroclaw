...
This page documents supported update and uninstall procedures for Mentat on macOS (OS X).
...
- Homebrew: `/opt/homebrew/bin/mentat` (Apple Silicon) or `/usr/local/bin/mentat` (Intel)
- Cargo/bootstrap/manual: `~/.cargo/bin/mentat`
...
```bash
mentat service stop || true
mentat service uninstall || true
```

Service artifacts removed by `service uninstall`:

- `~/Library/LaunchAgents/com.mentat.daemon.plist`
...
```bash
brew uninstall mentat
```

Cargo/bootstrap/manual (`~/.cargo/bin/mentat`):

```bash
cargo uninstall mentat || true
rm -f ~/.cargo/bin/mentat
```
...
```bash
command -v mentat || echo "mentat binary not found"
pgrep -fl mentat || echo "No running mentat process"
```
