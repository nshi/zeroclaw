...
[INFO] ✓ Audit logging enabled (~/.config/mentat/audit.log)
...
```bash
# Check what's active
$ mentat security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/mentat/audit.log
  → 47 events logged today

# Disable sandbox explicitly (writes to config)
$ mentat config set security.sandbox.enabled false

# Enable specific backend
$ mentat config set security.sandbox.backend firejail

# Adjust limits
$ mentat config set security.resources.max_memory_mb 2048
```
...
✅ **Status visibility** — `mentat security --status`
