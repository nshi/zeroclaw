# Mentat Security Improvement Roadmap
...
Mentat already has **excellent application-layer security**:
...
## Comparison: Mentat vs PicoClaw vs Production Grade

| Feature | PicoClaw | Mentat Now | Mentat + Roadmap | Production Target |
|---------|----------|--------------|-------------------|-------------------|
...
| Security self-test (`mentat audit --check`) | 1 day | Low |
...
[security.audit]
enabled = true
log_path = "~/.config/mentat/audit.log"
sign_events = true
max_size_mb = 100
...
mentat security --check
# → ✓ Sandbox: Firejail active
# → ✓ Audit logging enabled (42 events today)
# → → Resource limits: 512MB mem, 50% CPU

# Audit log queries
mentat audit --user @alice --since 24h
mentat audit --risk high --violations-only
mentat audit --verify-signatures
...
**Mentat is already more secure than PicoClaw** with:
...
**By implementing this roadmap**, Mentat becomes:
...
**Value**: Transforms Mentat from "safe for testing" to "safe for production"
