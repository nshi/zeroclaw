...
```bash
# Check config
cat ~/.mentat/config.toml | grep -A 5 telegram
...
| Health check | <5s | `time mentat channel doctor` |
...
5. **Monitor logs** for any issues:
   ...
   mentat daemon
   # or
   RUST_LOG=info mentat channel start
   ```
...
- Issues: https://github.com/nshi/mentat/issues
