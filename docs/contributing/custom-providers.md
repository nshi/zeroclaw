...
Mentat supports custom API endpoints for both OpenAI-compatible and Anthropic-compatible providers.
...
# or: export MENTAT_API_KEY="your-api-key"
mentat agent
```

## llama.cpp Server (Recommended Local Setup)

Mentat includes a first-class local provider for `llama-server`:
...
```bash
mentat models refresh --provider llamacpp
mentat agent -m "hello"
```

You do not need to export `MENTAT_API_KEY=dummy` for this flow.

## SGLang Server

Mentat includes a first-class local provider for [SGLang](https://github.com/sgl-project/sglang):
...
```bash
mentat models refresh --provider sglang
mentat agent -m "hello"
```
...
Mentat includes a first-class local provider for [vLLM](https://docs.vllm.ai/):
...
```bash
mentat models refresh --provider vllm
mentat agent -m "hello"
```
