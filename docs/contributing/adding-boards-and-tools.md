# Adding Boards and Tools — Mentat Hardware Guide

This guide explains how to add new hardware boards and custom tools to Mentat.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.mentat/config.toml)
mentat peripheral add nucleo-f401re /dev/ttyACM0
mentat peripheral add arduino-uno /dev/cu.usbmodem12345
mentat peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, Mentat can index PDF files:
...
2. **Add to config** — `mentat peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `mentat peripheral list` | List configured boards |
| `mentat peripheral add <board> <path>` | Add board (writes config) |
| `mentat peripheral flash` | Flash Arduino firmware |
| `mentat peripheral flash-nucleo` | Flash Nucleo firmware |
| `mentat hardware discover` | List USB devices |
| `mentat hardware info` | Chip info via probe-rs |
