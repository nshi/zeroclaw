...
- variable: `HOMEBREW_CORE_BOT_FORK_REPO` (for example `mentat-release-bot/homebrew-core`)
...
- variable: `SCOOP_BUCKET_REPO` (for example `nshi/scoop-mentat`)

Workflow guardrails:

- release tag must be `vX.Y.Z` format
- Windows binary SHA256 extracted from `SHA256SUMS` release asset
- manifest pushed to `bucket/mentat.json` in the Scoop bucket repo
...
- PKGBUILD and .SRCINFO pushed to AUR `mentat` package
