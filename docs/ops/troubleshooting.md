...
- `cargo check` / `cargo build` appears stuck at `Checking mentat` for a long time
- repeated `Blocking waiting for file lock on package cache` or `build directory`

Why this happens in Mentat:
...
### `mentat` command not found after install

Symptom:

- install succeeds but shell cannot find `mentat`
...
- stop extra `mentat daemon` / `mentat channel start` processes
