...
_ensure_bash() {
  ...
  if _have_cmd apk; then _run_privileged apk add --no-cache bash
    ...
    if _is_container_runtime; then
      _PACMAN_CFG="$(mktemp /tmp/mentat-pacman.XXXXXX.conf)"
...
usage() {
  cat <<'USAGE'
Mentat installer — one-click bootstrap

Usage:
  ./install.sh [options]

The installer builds Mentat, configures your provider and API key,
...
Examples:
  ...
  curl -fsSL https://github.com/nshi/mentat/install.sh | bash
...
Environment:
  MENTAT_CONTAINER_CLI     Container CLI command (default: docker; auto-fallback: podman)
  MENTAT_DOCKER_DATA_DIR   Host path for Docker config/workspace persistence
  MENTAT_DOCKER_IMAGE      Docker image tag to build/run (default: mentat-bootstrap:local)
  MENTAT_API_KEY           Used when --api-key is not provided
  MENTAT_PROVIDER          Used when --provider is not provided (default: openrouter)
  MENTAT_MODEL             Used when --model is not provided
  MENTAT_CARGO_FEATURES    Extra cargo features for source builds (comma/space separated)
  MENTAT_BOOTSTRAP_MIN_RAM_MB   Minimum RAM threshold for source build preflight (default: 2048)
  MENTAT_BOOTSTRAP_MIN_DISK_MB  Minimum free disk threshold for source build preflight (default: 6144)
  MENTAT_DISABLE_ALPINE_AUTO_DEPS
...
should_attempt_prebuilt_for_resources() {
  ...
  min_ram_mb="${MENTAT_BOOTSTRAP_MIN_RAM_MB:-2048}"
  min_disk_mb="${MENTAT_BOOTSTRAP_MIN_DISK_MB:-6144}"
...
resolve_asset_url() {
  local asset_name="$1"
  local api_url="https://api.github.com/repos/nshi/mentat/releases"
...
install_prebuilt_binary() {
  ...
  asset_name="mentat-${target}.tar.gz"
  ...
  if [[ -z "$archive_url" ]]; then
    archive_url="https://github.com/nshi/mentat/releases/latest/download/${asset_name}"
  fi

  temp_dir="$(mktemp -d -t mentat-prebuilt-XXXXXX)"
  ...
  extracted_bin="$temp_dir/mentat"
  if [[ ! -x "$extracted_bin" ]]; then
    extracted_bin="$(find "$temp_dir" -maxdepth 2 -type f -name mentat -perm -u+x | head -n 1 || true)"
  fi
  if [[ -z "$extracted_bin" || ! -x "$extracted_bin" ]]; then
    warn "Archive did not contain an executable mentat binary."
    rm -rf "$temp_dir"
    return 1
  fi
  ...
  mkdir -p "$install_dir"
  install -m 0755 "$extracted_bin" "$install_dir/mentat"
  rm -rf "$temp_dir"

  step_ok "Installed pre-built binary to $install_dir/mentat"
...
run_pacman() {
  ...
  local pacman_rc=0
  pacman_cfg_tmp="$(mktemp /tmp/mentat-pacman.XXXXXX.conf)"
...
MSG
      ...
      _xcode_test_file="$(mktemp /tmp/mentat-xcode-check.XXXXXX.c)"
...
prompt_api_key() {
  ...
  if [[ -n "$api_key_input" ]]; then
    ...
    warn "No API key entered — you can configure it later with mentat onboard"
    SKIP_ONBOARD=true
  fi
}
...
run_guided_installer() {
  ...
  echo
  echo -e "  ${BOLD_BLUE}${CRAB} Mentat Guided Installer${RESET}"
...
ensure_default_config_and_workspace() {
  ...
  #
  # $1 — config directory  (e.g. ~/.mentat or $docker_data_dir/.mentat)
  # $2 — workspace directory (e.g. ~/.mentat/workspace or $docker_data_dir/workspace)
...
# Mentat configuration — generated by install.sh
# Edit this file or run 'mentat onboard' to reconfigure.
...
resolve_container_cli() {
  local requested_cli
  requested_cli="${MENTAT_CONTAINER_CLI:-docker}"
  ...
  if [[ "$requested_cli" != "docker" ]]; then
    error "Set MENTAT_CONTAINER_CLI to an installed Docker-compatible CLI (e.g., docker or podman)."
  else
    error "Install Docker, install podman, or set MENTAT_CONTAINER_CLI to an available Docker-compatible CLI."
  fi
  exit 1
}
...
run_docker_bootstrap() {
  ...
  docker_image="${MENTAT_DOCKER_IMAGE:-mentat-bootstrap:local}"
  fallback_image="ghcr.io/nshi/mentat:latest"
  if [[ "$TEMP_CLONE" == true ]]; then
    default_data_dir="$HOME/.mentat-docker"
  else
    default_data_dir="$WORK_DIR/.mentat-docker"
  fi
  docker_data_dir="${MENTAT_DOCKER_DATA_DIR:-$default_data_dir}"
  DOCKER_DATA_DIR="$docker_data_dir"

  mkdir -p "$docker_data_dir/.mentat" "$docker_data_dir/workspace"
  ...
  if [[ "$SKIP_BUILD" == false ]]; then
    ...
    if ! "$CONTAINER_CLI" image inspect "$docker_image" >/dev/null 2>&1; then
      ...
      info "Pulling official Mentat image ($fallback_image)"
  ...
  config_mount="$docker_data_dir/.mentat:/mentat-data/.mentat"
  workspace_mount="$docker_data_dir/workspace:/mentat-data/workspace"
  ...
  if [[ ${#onboard_cmd[@]} -gt 0 ]]; then
    ...
    info "Docker image ready. Run mentat onboard inside the container to configure."
  fi
...
ROOT_DIR="$SCRIPT_DIR"
REPO_URL="https://github.com/nshi/mentat.git"
ORIGINAL_ARG_COUNT=$#
GUIDED_MODE="auto"
...
CONTAINER_CLI="${MENTAT_CONTAINER_CLI:-docker}"
API_KEY="${MENTAT_API_KEY:-}"
PROVIDER="${MENTAT_PROVIDER:-openrouter}"
MODEL="${MENTAT_MODEL:-}"
CARGO_FEATURES_INPUT="${MENTAT_CARGO_FEATURES:-}"
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  if [[ "$OS_NAME" == "Linux" && -z "${MENTAT_DISABLE_ALPINE_AUTO_DEPS:-}" ]] && have_cmd apk; then
    find_missing_alpine_prereqs
    if [[ ${#ALPINE_MISSING_PKGS[@]} -gt 0 && "$INSTALL_SYSTEM_DEPS" == false ]]; then
      ...
      info "Auto-enabling system dependency installation (set MENTAT_DISABLE_ALPINE_AUTO_DEPS=1 to disable)."
      INSTALL_SYSTEM_DEPS=true
    fi
  fi
...
if [[ ! -f "$WORK_DIR/Cargo.toml" ]]; then
  if [[ -f "$(pwd)/Cargo.toml" ]]; then
    ...
    TEMP_DIR="$(mktemp -d -t mentat-bootstrap-XXXXXX)"
...
echo
echo -e "  ${BOLD_BLUE}${CRAB} Mentat Installer${RESET}"
...
EXISTING_VERSION=""
INSTALL_MODE="fresh"
if have_cmd mentat; then
  EXISTING_VERSION="$(mentat --version 2>/dev/null | awk '{print $NF}' || true)"
  INSTALL_MODE="upgrade"
elif [[ -x "$HOME/.cargo/bin/mentat" ]]; then
  EXISTING_VERSION="$("$HOME/.cargo/bin/mentat" --version 2>/dev/null | awk '{print $NF}' || true)"
  INSTALL_MODE="upgrade"
fi
...
if [[ "$INSTALL_MODE" == "upgrade" && -n "$EXISTING_VERSION" ]]; then
  step_dot "Existing Mentat installation detected, upgrading from v${EXISTING_VERSION}"
elif [[ "$INSTALL_MODE" == "upgrade" ]]; then
  step_dot "Existing Mentat installation detected, upgrading"
fi
...
if [[ "$DOCKER_MODE" == true ]]; then
  ...
  echo
  echo -e "${BOLD}Your containerized Mentat data is persisted under:${RESET}"
  echo -e "  ${DIM}$DOCKER_DATA_DIR${RESET}"
  echo
  echo -e "${BOLD}Dashboard URL:${RESET} ${BLUE}http://127.0.0.1:42617${RESET}"
  echo
  echo -e "${BOLD}Next steps:${RESET}"
  echo -e "  ${DIM}mentat status${RESET}"
  echo -e "  ${DIM}mentat agent -m \"Hello, Mentat!\"${RESET}"
  echo -e "  ${DIM}mentat gateway${RESET}"
  echo
  echo -e "${BOLD}Docs:${RESET} ${BLUE}https://github.com/nshi/mentat/docs${RESET}"
  exit 0
fi
...
echo
echo -e "${BOLD_BLUE}[2/3]${RESET} ${BOLD}Installing Mentat${RESET}"
if [[ -n "$TARGET_VERSION" ]]; then
  step_dot "Installing Mentat v${TARGET_VERSION}"
fi
if [[ "$SKIP_BUILD" == false ]]; then
...
if [[ "$SKIP_INSTALL" == false ]]; then
  step_dot "Installing mentat to cargo bin"

  # Clean up stale cargo install tracking from the old "mentat" package name
  # (renamed to "mentat"). Without this, `cargo install mentat` from
  # crates.io fails with "binary already exists as part of `mentat`".
  if have_cmd cargo; then
    if [[ -f "$HOME/.cargo/.crates.toml" ]] && grep -q '^"mentat ' "$HOME/.cargo/.crates.toml" 2>/dev/null; then
      step_dot "Removing stale cargo tracking for old 'mentat' package name"
      cargo uninstall mentat 2>/dev/null || true
    fi
  fi

  cargo install --path "$WORK_DIR" --force --locked "${CARGO_FEATURE_ARGS[@]}"
  step_ok "Mentat installed"

  # Sync binary to ~/.local/bin so PATH lookups find the fresh version
  if [[ -d "$HOME/.local/bin" ]]; then
    cp -f "$HOME/.cargo/bin/mentat" "$HOME/.local/bin/mentat" 2>/dev/null && \
...
DESKTOP_DOWNLOAD_URL="https://github.com/nshi/mentat/download"
...
if [[ "$DEVICE_CLASS" == "desktop" ]]; then
  ...
  case "$OS_NAME" in
    Darwin)
      if [[ -d "/Applications/Mentat.app" ]] || [[ -d "$HOME/Applications/Mentat.app" ]]; then
        DESKTOP_APP_DETECTED=true
        step_ok "Companion app found (Mentat.app)"
      fi
      ;;
    Linux)
      if have_cmd mentat-desktop; then
        DESKTOP_APP_DETECTED=true
        step_ok "Companion app found (mentat-desktop)"
      elif [[ -x "$HOME/.local/bin/mentat-desktop" ]]; then
        DESKTOP_APP_DETECTED=true
        step_ok "Companion app found (~/.local/bin/mentat-desktop)"
      fi
      ;;
  esac
  ...
  if [[ "$DESKTOP_APP_DETECTED" == false ]]; then
    ...
    echo -e "  Menu bar access to your Mentat agent."
    ...
    echo -e "  ${DIM}Or run: mentat desktop --install${RESET}"
  fi
...
MENTAT_BIN=""
if [[ -x "$HOME/.cargo/bin/mentat" ]]; then
  MENTAT_BIN="$HOME/.cargo/bin/mentat"
elif [[ -x "$WORK_DIR/target/release/mentat" ]]; then
  MENTAT_BIN="$WORK_DIR/target/release/mentat"
elif have_cmd mentat; then
  MENTAT_BIN="mentat"
fi
...
if [[ "$SKIP_ONBOARD" == false && -n "$MENTAT_BIN" ]]; then
  if [[ -n "$API_KEY" ]]; then
    step_dot "Configuring provider: ${PROVIDER}"
    ONBOARD_CMD=("$MENTAT_BIN" onboard --api-key "$API_KEY" --provider "$PROVIDER")
    if [[ -n "$MODEL" ]]; then
      ONBOARD_CMD+=(--model "$MODEL")
    fi
    if "${ONBOARD_CMD[@]}" 2>/dev/null; then
      ...
      step_fail "Provider configuration failed — run mentat onboard to retry"
    fi
  elif [[ "$PROVIDER" == "ollama" ]]; then
    step_dot "Configuring Ollama (no API key needed)"
    if "$MENTAT_BIN" onboard --provider ollama 2>/dev/null; then
      step_ok "Ollama configured"
    else
      step_fail "Ollama configuration failed — run mentat onboard to retry"
    fi
  else
    ...
    if [[ -t 0 && -t 1 ]]; then
      ...
      step_dot "No API key provided — run mentat onboard to configure"
    fi
  fi
elif [[ "$SKIP_ONBOARD" == true ]]; then
  step_dot "Skipping configuration (run mentat onboard later)"
elif [[ -z "$MENTAT_BIN" ]]; then
  warn "Mentat binary not found — cannot configure provider"
fi
...
_native_config_dir="${MENTAT_CONFIG_DIR:-$HOME/.mentat}"
_native_workspace_dir="${MENTAT_WORKSPACE:-$_native_config_dir/workspace}"
ensure_default_config_and_workspace "$_native_config_dir" "$_native_workspace_dir" "$PROVIDER"

# --- Gateway service management ---
if [[ -n "$MENTAT_BIN" ]]; then
  # Try to install and start the gateway service
  step_dot "Checking gateway service"
  if "$MENTAT_BIN" service install 2>/dev/null; then
    step_ok "Gateway service installed"
    if "$MENTAT_BIN" service restart 2>/dev/null; then
      step_ok "Gateway service restarted"

    else
      step_fail "Gateway service restart failed — re-run with mentat service start"
    fi
  else
    step_dot "Gateway service not installed (run mentat service install later)"
  fi

  # --- Post-install doctor check ---
  step_dot "Running doctor to validate installation"
  if "$MENTAT_BIN" doctor 2>/dev/null; then
    step_ok "Doctor complete"
  else
    warn "Doctor reported issues — run mentat doctor --fix to resolve"
  fi
fi
...
INSTALLED_VERSION=""
if [[ -n "$MENTAT_BIN" ]]; then
  INSTALLED_VERSION="$("$MENTAT_BIN" --version 2>/dev/null | awk '{print $NF}' || true)"
fi

# --- Success banner ---
echo
if [[ -n "$INSTALLED_VERSION" ]]; then
  echo -e "${BOLD_BLUE}${CRAB} Mentat installed successfully (Mentat ${INSTALLED_VERSION})!${RESET}"
else
  echo -e "${BOLD_BLUE}${CRAB} Mentat installed successfully!${RESET}"
fi

if [[ -x "$HOME/.cargo/bin/mentat" ]] && ! have_cmd mentat; then
  echo
  warn "mentat is installed in $HOME/.cargo/bin, but that directory is not in PATH for this shell."
...
if [[ "$DEVICE_CLASS" == "desktop" ]]; then
  if [[ "$DESKTOP_APP_DETECTED" == true ]]; then
    echo -e "  ${DIM}mentat desktop${RESET}                ${DIM}# Launch the menu bar app${RESET}"
  else
    echo -e "  ${DIM}mentat desktop --install${RESET}      ${DIM}# Download the companion app${RESET}"
  fi
fi
echo
echo -e "${BOLD}Docs:${RESET} ${BLUE}https://github.com/nshi/mentat/docs${RESET}"
